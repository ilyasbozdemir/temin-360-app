...
import { resolveTemplateConfig, TEMPLATE_OPTIONS } from '../templateResolver'
...
export function useDocumentPreviewData({
  ...
  const [selectedDocId, setSelectedDocId] = useState<string>(() => documentId || 'ihtiyac-listesi')
  ...
  const initialPreloaded = documentPreloadService.getCachedDocument(resolvedId, activeDosyaId)
  ...
  const isDirty = Boolean(
    initialSnapshotRef.current && currentSnapshot !== initialSnapshotRef.current && !isLoading
  )
  ...
  useEffect(() => {
    ...
    let isMounted = true
    const preloaded = documentPreloadService.getCachedDocument(resolvedId, activeDosyaId)
    if (!preloaded) {
      setIsLoading(true)
    }
