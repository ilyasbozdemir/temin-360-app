...
export function DocumentPreviewHeader({
  ...
  const { isInQueue, toggleReadyToPrint, markAsPrinted, getDocumentStatus } = usePrintQueueStore()

  const isQueued = targetDosyaId && docKey ? isInQueue(targetDosyaId, docKey) : false
  const docStatus = targetDosyaId && docKey ? getDocumentStatus(targetDosyaId, docKey) : 'draft'
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="flex items-center bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60 rounded-xl p-0.5">
          <button
            onClick={() => {
              setZoomMode('manual')
              setManualZoom((prev) => Math.max(0.4, Number((prev - 0.1).toFixed(1))))
            }}
          ...
          <button
            onClick={() => {
              setZoomMode('manual')
              setManualZoom((prev) => Math.min(2.0, Number((prev + 0.1).toFixed(1))))
            }}
            className="p-1 hover:bg-white dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg transition-colors cursor-pointer"
