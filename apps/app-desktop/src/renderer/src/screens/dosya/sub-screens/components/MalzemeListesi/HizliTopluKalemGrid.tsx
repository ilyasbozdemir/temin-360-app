...
const PRESET_PACKAGES = [
  ...
  },
  {
    ...
    birim: "m²",
    items: [
      { kalem_adi: "İç Cephe Alçı Sıva Tamiratı ve Silikonlu Plastik Boya", miktar: 250, birim: "m²", aciklama: "Yüzey temizliği, astar ve 2 kat silikonlu boya" },
...
export function HizliTopluKalemGrid({
  ...
  const unitOptions =
    units.length > 0 ? units.map((u: any) => u.ad || u.name || u) : DEFAULT_UNITS;

  // Table Rows
  const [rows, setRows] = useState<HizliKalemRow[]>([
    { id: "row-1", tipi: defaultTipi, kalem_adi: "", miktar: 1, birim: defaultBirim, tasinir_kodu: "", okas_kodu: "", kdv_orani: 20, aciklama: "" },
    { id: "row-2", tipi: defaultTipi, kalem_adi: "", miktar: 1, birim: defaultBirim, tasinir_kodu: "", okas_kodu: "", kdv_orani: 20, aciklama: "" },
    { id: "row-3", tipi: defaultTipi, kalem_adi: "", miktar: 1, birim: defaultBirim, tasinir_kodu: "", okas_kodu: "", kdv_orani: 20, aciklama: "" },
    { id: "row-4", tipi: defaultTipi, kalem_adi: "", miktar: 1, birim: defaultBirim, tasinir_kodu: "", okas_kodu: "", kdv_orani: 20, aciklama: "" },
    { id: "row-5", tipi: defaultTipi, kalem_adi: "", miktar: 1, birim: defaultBirim, tasinir_kodu: "", okas_kodu: "", kdv_orani: 20, aciklama: "" },
  ]);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (rowEl) {
        const targetIdx = parseInt(
          rowEl.getAttribute("data-grid-row-idx") || "-1",
          10,
        );
    ...
    const handleMouseUp = () => {
      if (dragFill && dragFill.isDragging) {
        ...
        setRows((prev) =>
          prev.map((r, idx) =>
            idx >= start && idx <= end ? { ...r, [field]: val } : r,
          ),
        );

        showToast(
          `Excel Doldurma: Satır ${start + 1} - ${end + 1} arası güncellendi.`,
        );
      }
      setDragFill(null);
    };
  ...
  const renderCellWithFillHandle = (
    ...
    const isHovered =
      hoveredCell?.rowIdx === rowIdx && hoveredCell?.field === field;
    const isDragSource =
      dragFill?.sourceRowIdx === rowIdx && dragFill?.sourceField === field;
    const isDragTarget =
      dragFill &&
    ...
    <div className="space-y-3.5 animate-in fade-in duration-200 select-none">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-blue-50/70 dark:bg-blue-950/30 border border-blue-200/60 dark:border-blue-900/30 rounded-xl text-[11px] text-blue-800 dark:text-blue-300">
        <div className="flex items-center gap-1.5">
          ...
          <span>
            <strong>Excel Sürükle-Doldur:</strong> Hücrenin sağ altındaki mavi kutucuğu aşağı sürükleyerek veya çift tıklayarak değeri alt satırlara çoğaltabilirsiniz. Sütun başlıklarındaki (
            <ArrowDownToLine className="w-3 h-3 inline text-blue-600 mx-0.5" />) butonuyla ilk satırı tüm sütuna kopyalayabilirsiniz. (Kısayol: <strong>Ctrl + D</strong>)
          </span>
        </div>
      </div>
      ...
      <div className="border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs bg-white dark:bg-slate-900">
            ...
            <thead className="bg-slate-100 dark:bg-slate-800/90 sticky top-0 z-10 text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider border-b border-slate-200 dark:border-slate-700">
              <tr>
                ...
                <th className="w-72 sm:w-80 px-3 py-2.5">
                  <span>Kalem / İmalat Adı <span className="text-red-500">*</span></span>
                </th>
                ...
                >
                  ...
                  <td className="px-1.5 py-1">
                    <input
                      ...
                      placeholder={`Örn: ${
                        idx === 0
                          ? isYapim ? "İç Cephe Boyası Yapılması" : "A4 Fotokopi Kağıdı 80gr"
                          : idx === 1
                          ? isYapim ? "Tavan Boyası Tamiratı" : "Plastik Telli Dosya"
        ...
        <div className="p-2.5 bg-slate-50/80 dark:bg-slate-800/40 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="text-[11px] text-slate-400 flex items-center gap-3">
            <span>✨ <strong>Ctrl+D:</strong> Üst satırı kopyalar</span>
            <span>🖱️ <strong>Çift Tık:</strong> Sütunu aşağı doldurur</span>
          </div>
        </div>
      </div>
