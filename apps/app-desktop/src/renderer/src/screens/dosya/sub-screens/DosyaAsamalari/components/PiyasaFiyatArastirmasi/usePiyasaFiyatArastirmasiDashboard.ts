...
export function usePiyasaFiyatArastirmasiDashboard(
  ...
  const [currentStep, setCurrentStep] = useState<1 | 2 | 3>(() => {
    ...
    const hasAnyBid = invitedFirms.some(
      (f: any) => f.teklif_toplami && f.teklif_toplami > 0,
    );
      ...
      if (!foundSablon) {
        for (const key of candidateKeys) {
          ...
          foundSablon = sablons.find((s: any) => {
            ...
            return (
              normSablonName.includes(normKey) || normKey.includes(normSablonName)
            );
          });
          if (foundSablon) break;
        }
  ...
  const findSablonForBelge = (belge: BelgeItem) => {
    ...
    let found = sablons.find((s: any) => {
      const normSablonName = normalizeForMatch(s.ad || s.dosya_adi || "");
      return (
        normSablonName.includes(normDocName) ||
