...
export function KazananKararPaneli({
  ...
  const isLowestBasis = !hesaplamaEsasi ||
        ...
        <div>
              ...
              ? (
                ...
                >
                  <option value="En Düşük fiyat esasına göre">
                    ⚙️ En Düşük Fiyat Esası
                  </option>
                  <option value="Ortalama fiyat esasına göre">
                    ⚙️ Ortalama Fiyat Esası
                  </option>
                </select>
              )
              : (
          ...
          <div className="text-sm font-extrabold text-slate-800 dark:text-slate-100 mt-0.5">
            {activeWinnerFirma
              ...
              : lowestBidFirm
              ? `${lowestBidFirm.unvan} (${
                isLowestBasis ? "En Düşük Teklif" : "En Uygun Teklif"
              })`
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {lowestBidFirm && (
          <button
            ...
            title={`${
              isLowestBasis ? "En düşük" : "En uygun"
            } teklif sahibi (${lowestBidFirm.unvan}) kazanan olarak atanır.`}
          >
            <Sparkles className="w-3.5 h-3.5 text-slate-950" />
            {isLowestBasis
              ? "En Düşük Teklifi Kazanan Yap"
              : "En Uygun Teklifi Kazanan Yap"}
          </button>
        )}
