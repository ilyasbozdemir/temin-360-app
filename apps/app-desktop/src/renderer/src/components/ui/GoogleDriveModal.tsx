...
export function GoogleDriveModal({ isOpen, onClose }: GoogleDriveModalProps): React.JSX.Element {
  ...
  const fetchDriveFiles = async (currentToken?: string) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        const validList = (res.files || []).filter((f: GDriveFile) => {
        ...
        if (validList.length === 0) {
          setStatusMsg({
            text: 'TEMIN_360_YEDEKLER klasöründe henüz proje yedek dosyası bulunamadı.',
            type: 'info'
          })
        }
      } else {
    ...
    window.electron?.ipcRenderer
      ...
      .then((settings) => {
        ...
        if (settings?.gdriveAccessToken) {
          const clean = settings.gdriveAccessToken
            .trim()
            .replace(/^["']|["']$/g, '')
            .replace(/^Bearer\s+/i, '')
            .replace(/[\r\n\s]+/g, '')
        ...
        if (settings?.gdriveRefreshToken) {
          const cleanRefresh = settings.gdriveRefreshToken
            .trim()
            .replace(/^["']|["']$/g, '')
            .replace(/[\r\n\s]+/g, '')
  ...
  const handleStartGoogleOAuth = async () => {
    const cleanClientId = clientId
      ...
      .replace(/^["']|["']$/g, '')
      .replace(/[\r\n\s]+/g, '')
    const cleanClientSecret = clientSecret
      .trim()
      .replace(/^["']|["']$/g, '')
      .replace(/[\r\n\s]+/g, '')
  ...
  const handleOpenGoogleAuth = () => {
    ...
    setStatusMsg({
      text: "OAuth Playground tarayıcıda açıldı. Sağ üstteki ⚙️ Dişli simgesinden kendi Client ID ve Secret'ınızı girip Drive API v3 seçerek kalıcı yetki alabilirsiniz.",
      type: 'info'
    })
  }
  ...
  const handleImportClientJson = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (importedCount > 0) {
          setStatusMsg({
            text: "✅ JSON dosyasından Client ID ve Client Secret başarıyla okundu! Aşağıdaki 'Kaydet ve Kalıcı Modu Aktif Et' butonuna basarak kaydedebilirsiniz.",
            type: 'success'
          })
          setAuthTab('api')
        } else {
          setStatusMsg({
            text: 'JSON dosyasında geçerli client_id veya client_secret bulunamadı.',
            type: 'error'
          })
        }
      } catch {
  ...
  const handleSaveApiSettings = async () => {
    ...
    const cleanClientSecret = clientSecret
      ...
      .replace(/^["']|["']$/g, '')
      .replace(/[\r\n\s]+/g, '')
    const cleanRefresh = refreshToken
      .trim()
      .replace(/^["']|["']$/g, '')
      .replace(/[\r\n\s]+/g, '')
    ...
    if (!cleanClientId || !cleanClientSecret) {
      setStatusMsg({
        text: "Lütfen Client ID ve Client Secret alanlarını doldurun veya 'client_secret.json Yükle' butonunu kullanın.",
        type: 'error'
      })
      return
    }
  ...
  const handleSaveManualToken = async () => {
    ...
    try {
      ...
      setIsSavedToken(true)
      setStatusMsg({
        text: '✅ Manuel Access Token kaydedildi. Google Drive dosyalarınız çekiliyor...',
        type: 'success'
      })
  ...
  const handleUploadCurrentFile = async () => {
    ...
    try {
      ...
      if (res.success) {
        setStatusMsg({
          text: res.message || 'Dosya Google Drive hesabınıza başarıyla yüklendi.',
          type: 'success'
        })
        fetchDriveFiles(cleanToken || undefined)
      } else {
  ...
  const handleDownloadFile = async (file: GDriveFile, overwriteActive: boolean = true) => {
    if (overwriteActive) {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-1 border-b border-slate-200/50 dark:border-slate-800/50">
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400">
                Aşağıdaki iki yöntemden birini seçip kaydedin; girdiğiniz yöntem otomatik aktif
                olur.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            >
              ...
              <div className="space-y-0.5 min-w-0">
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight">
                  Client ID & Secret ile kalıcı, süresiz otomatik yenileme (24 saat sınırı yok).
                </p>
              </div>
            </button>
            ...
            >
              ...
              <div className="space-y-0.5 min-w-0">
                ...
                <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight">
                  Sadece tek bir Access Token (ya29...) ile anında test (~1 saat geçerli).
                </p>
              </div>
            </button>
          </div>
            ...
            <div className="space-y-3 p-3.5 bg-white/70 dark:bg-slate-900/70 rounded-xl border border-blue-200/70 dark:border-blue-900/50">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    >
                      {showClientSecret ? <EyeOff size={13} /> : <Eye size={13} />}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3.5 bg-gradient-to-r from-blue-50 via-indigo-50 to-emerald-50 dark:from-blue-950/40 dark:via-indigo-950/40 dark:to-emerald-950/40 border border-blue-200 dark:border-blue-800/60 rounded-xl space-y-2">
                ...
                <p className="text-[10px] text-center text-slate-600 dark:text-slate-400 leading-snug">
                  ✨ <strong>24 saat sınırı yok:</strong> Butona tıkladığınızda tarayıcınız açılır;
                  Google hesabınıza onay verdiğinizde kalıcı yetki otomatik alınır ve yedekleme
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-gradient-to-br from-blue-500/10 to-indigo-500/10 dark:from-blue-950/40 dark:to-indigo-950/40 border border-blue-200/60 dark:border-blue-800/40 p-4 rounded-2xl space-y-2.5">
            ...
            <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
              Mevcut çalışma dosyanızı tarih damgasıyla (.dtal) doğrudan Google Drive hesabınıza
              yedek olarak aktarır.
            </p>
            <Button
          ...
          <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-slate-950 max-h-64 overflow-y-auto">
            {isLoadingList ? (
              <div className="p-8 text-center text-xs text-slate-500 dark:text-slate-400 flex items-center justify-center gap-2">
                <RefreshCw size={16} className="animate-spin text-blue-500" /> Dosyalar
                listeleniyor...
              </div>
            ) : files.length === 0 ? (
