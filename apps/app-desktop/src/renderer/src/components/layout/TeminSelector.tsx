...
export function TeminSelector(): React.JSX.Element {
  ...
  const filteredDosyalar = dosyalar.filter((d) => {
    ...
    if (subFilter === "mode_default") {
      ...
      if (isIhale || (d.tur !== "hizmet" && d.tur !== "yapim_isi")) {
        return false;
      }
      ...
      if (
        !isIhale ||
        (!d.ihale_tipi?.includes("Açık") &&
          !d.ihale_sekli?.toLowerCase().includes("açık"))
      ) return false;
    } else if (subFilter === "ihale_pazarlik") {
      if (
        !isIhale ||
        (!d.ihale_tipi?.includes("Pazarlık") &&
          !d.ihale_sekli?.toLowerCase().includes("pazarlık"))
      ) return false;
    } else if (subFilter === "ihale_yapim") {
      const isYapim = d.tur === "yapim_isi" || d.tur === "hakedis" ||
        d.ihale_sekli?.toLowerCase().includes("yapım") ||
        d.ihale_sekli?.toLowerCase().includes("hakedis") ||
        d.ihale_tipi === "Hakediş";
      if (!isYapim) return false;
    } else if (subFilter === "ihale_hizmet") {
      const isHizmet = d.tur === "hizmet" ||
        d.ihale_sekli?.toLowerCase().includes("hizmet");
  ...
  const getMevzuatBadgeInfo = (dosya: any) => {
    ...
    if (itemIsIhale) {
      if (
        sekli.includes("açık") || tipi.includes("açık") || sekli.includes("19")
      ) {
        return {
      ...
      if (
        sekli.includes("pazarlık") || tipi.includes("pazarlık") ||
        sekli.includes("21")
      ) {
        return {
  ...
  return (
    <>
      <div
        className="relative max-w-full min-w-0 flex justify-center"
        ref={containerRef}
      >
        {selectedDosya
          ? (
            ...
            <div className="flex items-center justify-between gap-3 p-1.5 bg-slate-100/70 dark:bg-slate-950/60 rounded-xl border border-slate-200/60 dark:border-slate-800 mb-2.5">
              <div className="flex items-center gap-1.5">
                ...
                >
                  ...
                  >
                    {dosyalar.filter((d) =>
                      d.is_deleted !== 1 && !isIhaleOrYapim(d)
                    ).length}
                  </span>
                </button>
              ...
              >
                ...
                <span>
                  {isDt ? "Yeni Doğrudan Temin" : "Yeni İhale Dosyası"}
                </span>
              </button>
            </div>
              ...
              {isDt
                ? (
                  <>
                    ...
                    >
                      Tüm Doğrudan Teminler ({dosyalar.filter((d) =>
                        d.is_deleted !== 1 && !isIhaleOrYapim(d)
                      ).length})
                    </button>
                    <button
                ...
                : (
                  <>
                    ...
                    >
                      Tüm İhale Dosyaları ({dosyalar.filter((d) =>
                        d.is_deleted !== 1 && isIhaleOrYapim(d)
                      ).length})
                    </button>
                    <button
                ...
                ? (
                  <div className="p-8 text-center text-xs text-slate-400 flex flex-col items-center gap-3">
                    ...
                    >
                      ...
                      {isDt
                        ? "Yeni Doğrudan Temin Oluştur"
                        : "Yeni İhale Dosyası Oluştur"}
                    </button>
                  </div>
                )
                : (
