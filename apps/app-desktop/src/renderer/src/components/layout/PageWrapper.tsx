...
export function PageWrapper(): React.ReactNode {
    ...
    const ensureInteractivity = () => {
      ...
      if (!hasVisibleModal) {
        ...
        if (
          ...
          window.getComputedStyle(document.documentElement).pointerEvents ===
            "none"
        ) {
    ...
    const handleUserInteraction = (e: Event) => {
      ...
      if (
        ...
        const isReadOnly =
          ...
          e.target.readOnly;
        if (
          document.activeElement !== e.target && !e.target.disabled &&
          !isReadOnly
        ) {
          e.target.focus();
        }
  ...
  useEffect(() => {
    const handleQuitRequest = async () => {
      ...
      try {
        const s = await window.electron?.ipcRenderer?.invoke("db:get-settings");
        if (
          s?.closeActionRemember === "true" && s?.closeActionPreference &&
    ...
    const interval = setInterval(() => {
      ...
      setExpiredPaths((prev) => {
        ...
        for (const tab of tabs) {
          ...
          const isExpired = !isActive && tab.path !== "/" &&
            now - lastActiveTime > 5 * 60 * 1000;
  ...
  useEffect(() => {
    ...
    const handleDteFileOpen = async (filePath: string) => {
      ...
      const fileBaseName = filePath.split("\\").pop()?.split("/").pop() ||
        "veri";
    ...
    const initWorkspace = async (): Promise<void> => {
      ...
      try {
        const initialFile = await window.electron?.ipcRenderer.invoke(
          "get-initial-file",
        );
        if (initialFile) {
          console.log(
            "[PageWrapper] Başlangıçta çift tıklanan dosya açılıyor:",
            initialFile,
          );
      ...
      try {
        ...
        const dbIsOpen = res && res.institutionName &&
          !res.institutionName.includes("Hata");
        ...
        if (!dbIsOpen && targetPath && isMounted) {
          ...
          const recent = await window.electron?.ipcRenderer.invoke(
