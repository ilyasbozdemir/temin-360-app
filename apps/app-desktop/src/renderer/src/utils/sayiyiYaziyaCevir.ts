 ...
 *   1           -> "BİR TL"
 *   1000        -> "BİN TL" (BİR BİN DEĞİL)
 *   100         -> "YÜZ TL" (BİR YÜZ DEĞİL)
 */
...
export function numberToWords(n: number): string {
  ...
  // Sayıyı 3'erli basamak gruplarına ayır (birler, binler, milyonlar, ...)
  const groups: number[] = []
...
export function amountToWordsTL(
  ...
  if (isNaN(value) || value < 0) return 'GEÇERSİZ TUTAR'

  const tam = Math.floor(value)
  // Kuruş: ondalık kısmı 2 haneye yuvarlayıp tam sayıya çevir
  const kurus = Math.round((value - tam) * 100)

  let words = numberToWords(tam) + ` ${paraBirimi}`
  if (kurus > 0 || !sadeceKurusVarsaGoster) {
    if (kurus > 0) {
      words += ' ' + numberToWords(kurus) + ` ${altBirim}`
    }
  }
